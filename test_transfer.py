...
if __name__ == '__main__':
    ...
    parser.add_argument("--from-problem", type=str, default="maxcut")
    parser.add_argument("--to-problem", type=str, default="mis")
    ...
    n_layers = args.p
    from_problem = args.from_problem
    to_problem = args.to_problem
    epochs = args.epochs
    ...
    print("from problem", from_problem)
    print("to problem", to_problem)
    print("epochs", epochs)
    ...
    if use_g2v:
        to_test_embedding_path = f'./dataset_{args.dataset}/testset/embeddings.npy'
    else:
        to_test_embedding_path = f'./dataset_{args.dataset}/testset/{to_problem}_unihetco_embeddings.pkl'
    ...
    if use_g2v or use_uni:
        if use_uni:
            with open(to_test_embedding_path, "rb") as infile:
                test_embeddings = pickle.load(infile)
        else:
            test_embeddings = np.load(to_test_embedding_path)
    ...
    state_dict = torch.load(f"./models_{args.dataset}/{from_problem}_p={n_layers}_T={horizon}_epochs={epochs}_bs={batch_size}_g2v={use_g2v}_uni={use_uni}_normalize={normalize}_seed={seed}.pth", map_location=device)
    ...
    with open(f"./transfer_results/{from_problem}_to_{to_problem}_p={n_layers}_g2v={use_g2v}_uni={use_uni}.txt", "w") as outfile:
        outfile.write("=============================================\n")
        outfile.write(f"dataset {dataset}\n")
        outfile.write(f"n_layers {n_layers}\n")
        outfile.write(f"from problem {from_problem}\n")
        outfile.write(f"to problem {to_problem}\n")
        outfile.write(f"epochs {epochs}\n")
        outfile.write(f"batch_size {batch_size}\n")
        outfile.write(f"use_g2v {use_g2v}\n")
        outfile.write(f"use_uni {use_uni}\n")
        outfile.write(f"horizon {horizon}\n")
        outfile.write(f"normalize {normalize}\n")
        outfile.write(f"test_steps {test_steps}\n")
        outfile.write(f"seed {seed}\n")
        outfile.write(f" > Feasible Ratio: {(sum(all_feasible_ratios)/ len(all_feasible_ratios)):.4f}\n")
        outfile.write(f" > Percent Optimal: {(sum(percent_optimal_list)/ len(percent_optimal_list)):.4f}\n")
        outfile.write(f" > Approximation Ratio: {(sum(approximation_ratio_list)/ len(approximation_ratio_list)):.4f}\n")
